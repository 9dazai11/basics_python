...
import xmlrpc.client  # Импортируем модуль для работы с XML-RPC

# Создаем XML-RPC клиент
server_url = "http://localhost:8000"  # URL вашего XML-RPC сервера
...
def setup_matrix_input():
    ...
    def execute_addition():
        ...
        if matrix1 and matrix2:
            result = client.add_matrices(matrix1, matrix2)  # Вызов метода на сервере
    ...
    def execute_multiplication():
        ...
        matrix2 = create_matrix(matrix2_entries, rows2, cols2)
        if matrix1 and matrix2:
            result = client.multiply_matrices(matrix1, matrix2)  # Вызов метода на сервере
    ...
    def execute_transpose_matrix1():
        matrix1 = create_matrix(matrix1_entries, rows1, cols1)
        if matrix1:
            result = client.transpose_matrix(matrix1)  # Вызов метода на сервере
            display_matrix(result, result_label, "Результат транспонирования первой матрицы")

    def execute_determinant_matrix1():
        matrix1 = create_matrix(matrix1_entries, rows1, cols1)
        if matrix1:
            result = client.determinant(matrix1)  # Вызов метода на сервере
    ...
    def execute_scalar_multiply_matrix1():
        ...
        if matrix1:
            result = client.scalar_multiply(matrix1, scalar)  # Вызов метода на сервере
    ...
    def execute_transpose_matrix2():
        matrix2 = create_matrix(matrix2_entries, rows2, cols2)
        if matrix2:
            result = client.transpose_matrix(matrix2)  # Вызов метода на сервере
            display_matrix(result, result_label, "Результат транспонирования второй матрицы")

    def execute_determinant_matrix2():
        matrix2 = create_matrix(matrix2_entries, rows2, cols2)
        if matrix2:
            result = client.determinant(matrix2)  # Вызов метода на сервере
    ...
    def execute_scalar_multiply_matrix2():
        ...
        if matrix2:
            result = client.scalar_multiply(matrix2, scalar)  # Вызов метода на сервере
